...
ABN.is_dark = function(card)
  ...
  if (Card.is_suit_shade and card:is_suit_shade("dark")) or card:is_suit("Spades") or card:is_suit("Clubs") or card:is_suit('bunc_Halberds') or card:is_suit('paperback_Crowns') or card:is_suit("abn_Penumbra") or card:is_suit("abn_Bow") or card:is_suit("abn_Chalice") or card:is_suit("abn_Sword") or card:is_suit("abn_Shield") or card:is_suit("abn_Leaf") or card:is_suit("abn_Acorn") or card:is_suit("abn_Anchor") or card:is_suit("abn_Megido") then
    return true
  end
  return false
end
...
ABN.is_light = function(card)
  if SMODS.has_no_suit(card) then return false end
  if (Card.is_suit_shade and card:is_suit_shade("light")) or card:is_suit("Diamonds") or card:is_suit("Hearts") or card:is_suit('bunc_Fleurons') or card:is_suit('paperback_Stars') or card:is_suit("abn_Snow") or card:is_suit("abn_Tie") or card:is_suit("abn_Coin") or card:is_suit("abn_Baton") or card:is_suit("abn_Rose") or card:is_suit("abn_Florette") or card:is_suit("abn_Bell") or card:is_suit("abn_Arrow") then
    return true
  end
  return false
end
...
ABN.enh_stickers_list = {
  ...
  "abn_stk_infra",
  "abn_stk_hazard",
  --"abn_stk_contagion_seal", --while this is technically an enhancement, it will not count an "enhancement sticker" so that it wont remove nor get removed by enh stickers.
  "abn_stk_contagion_bonus",
  "abn_stk_contagion_mult",
  "abn_stk_monitor",
...
--- @param key string enhancement key ("m_bonus") or a sticker key ("abn_stk_bonus").
--- @return table|nil  { mod = "...", sticker = "...", enhancement = "..." } or nil
function ABN.get_enh_sticker(key)
  if not key or not ABN.maps_enhancement_to_sticker then return nil end

  local data = ABN.maps_enhancement_to_sticker[key]
  if data then
    return {
      mod = data.mod,
  ...
  for enh, info in pairs(ABN.maps_enhancement_to_sticker) do
    if info.sticker == key then
      return {
        mod = info.mod,
...
--- Returns the sticker key if Joker has enhancement sticker
--- @param card table e.g. joker you want to check
--- @return string|nil "abn_stk_darkner" or nil if none
