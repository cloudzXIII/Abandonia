...
function ABN.retrieve_joker_text(joker, descip, name, colours_only)
  local colours = {}
  local function get_colours(s)
  ...
  if colours_only then
    local loc = G.localization.descriptions["Joker"][joker.config.center.key]
    if loc then
      ...
      if loc.text then scan(loc.text) end
    end
    return colours
  end
    ...
    if joker.generate_UIBox_ability_table then
      if not joker.ability_UIBox_table then -- Removing this check causes memory leaks
        ...
        if center then
          ...
          text = ABN.retrieve_joker_text(joker)
        end
      end
