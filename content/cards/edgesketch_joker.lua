SMODS.Joker {
  ...
  calculate = function(self, card, context)
    if context.other_joker and not context.blueprint then
      if context.other_joker ~= card and context.other_joker == G.jokers.cards[#G.jokers.cards] then
        card.ability.extra.mult = card.ability.extra.mult + card.ability.extra.mult_gain
        return { message = localize('k_upgrade_ex'), colour = G.C.FILTER, card = card }
      end
      if context.other_joker ~= card and context.other_joker == G.jokers.cards[1] then
