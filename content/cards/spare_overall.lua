SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            ...
            if SMODS.has_enhancement(context.other_card, "m_mult") and ABN.is_even(context.other_card) then
                context.other_card.ability.perma_mult = (context.other_card.ability.perma_mult or 0) +
                    card.ability.extra.perma_mult
                triggered = true
            end
            if SMODS.has_enhancement(context.other_card, "m_bonus") and ABN.is_odd(context.other_card) then
