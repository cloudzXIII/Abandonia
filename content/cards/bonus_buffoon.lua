SMODS.Joker {
  ...
  in_pool = function(self)
    ...
    for _, card in ipairs(G.playing_cards) do
      if card.config.center ~= G.P_CENTERS.m_base then
        has_enhanced = true
        break
      end
    end
    ...
    return has_comedian and has_enhanced
  end,
  ...
  calculate = function(self, card, context)
    if context.individual and context.cardarea == G.play then
      ...
      if currentCard and currentCard.config.center and currentCard.config.center.set and currentCard.config.center.set == "Enhanced" then
        return { xmult = card.ability.extra.xmult }
      end
    end

    if context.end_of_round and context.game_over == false and context.main_eval and not context.blueprint then
      if SMODS.pseudorandom_probability(card, 'abn_buffoon', 1, card.ability.extra.odds) then
        SMODS.destroy_cards(card, nil, nil, true)
        G.GAME.BonusDied = true
        return {
          message = ("oops"),
          colour = G.C.BLUE
        }
