...
function set_consumeable_usage(card)
  local ret = scu(card)
  if card.config.center.set == 'calligraphy' then
    G.E_MANAGER:add_event(Event({
      ...
      func = function()
        G.E_MANAGER:add_event(Event({
          trigger = 'immediate',
          func = function()
            G.GAME.abn_last_calligraphy = card.config.center.key
            return true
          end
        }))
        return true
...
SMODS.Joker {
  ...
  loc_vars = function(self, info_queue, card)
    local last_key = G.GAME and G.GAME.abn_last_calligraphy
    local calligraphy_center = last_key and G.P_CENTERS[last_key] or nil

    if calligraphy_center then
      info_queue[#info_queue + 1] = calligraphy_center
    end

    local last_calligraphy_name = calligraphy_center and localize({ type = 'name_text', key = calligraphy_center.key, set = calligraphy_center.set }) or localize('k_none')
  ...
  in_pool = function(self, args)
    ...
    if G.playing_cards then
      for _, c in ipairs(G.playing_cards) do
        ...
        if has_stone and has_glass then break end
      end
    end

    return has_stone and has_glass
  end,

  calculate = function(self, card, context)
    if context.setting_blind and G.GAME.blind.name == 'Big Blind' and not context.blueprint then
      if G.GAME.abn_last_calligraphy and #G.consumeables.cards < G.consumeables.config.card_limit then
        G.E_MANAGER:add_event(Event({
          func = function()
            local calligraphy_card = create_card('calligraphy', G.consumeables, nil, nil, nil, nil, G.GAME.abn_last_calligraphy)
            calligraphy_card:add_to_deck()
            G.consumeables:emplace(calligraphy_card)
            return true
          end
        }))
        return {
          message = "+1 Calligraphy",
          colour = G.C.SECONDARY_SET.calligraphy,
        }
      end
    end
    ...
    if context.before and context.full_hand then
      ...
      if has_stone and has_glass then
        for _, sc in ipairs(context.full_hand) do
          if sc.config.center == G.P_CENTERS.m_stone or sc.config.center == G.P_CENTERS.m_glass then
