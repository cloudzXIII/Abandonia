SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if context.joker_main and context.scoring_name == card.ability.extra.poker_hand then
            ...
            for i, playing_card in ipairs(G.play.cards) do
                ...
                if playing_card.base.suit == "abn_Star" then has_penumbra = true end
                if playing_card.base.suit == "abn_Moon" then has_snow = true end
                if playing_card.base.suit == "abn_Crown" then has_bows = true end
                if playing_card.base.suit == "abn_Talon" then has_ties = true end
            end

            if has_ties and has_bows and has_snow and has_penumbra then
                local target_card = context.scoring_hand[1]
                local _rank = G.GAME.abn_rank_upgrades[target_card.base.value]
                ...
                ABN.level_up_rank(card, target_card.base.value, #context.scoring_hand)
            ...
            elseif has_hearts and has_clubs and has_diamonds and has_spades then
                if #G.play.cards == 6 then 
                    SMODS.smart_level_up_hand(nil, card.ability.extra.poker_hand, false, 1) 
                end
    ...
    in_pool = function(self, args)
        if G.GAME.hands["Four of a Kind"] and G.GAME.hands["Four of a Kind"].played > 0 then 
            return true 
        end
        return false
    end,
