...
SMODS.Consumable {
  ...
  loc_vars = function(self, info_queue, card)
    ...
    return { vars = { (card and card.ability.extra.amount or self.config.extra.amount), localize({ type = 'name_text', key = "m_abn_fossil", set = "Enhanced" }), localize({ type = 'name_text', key = "e_abn_sunscourge", set = "Edition" }) } }
  end,
    ...
    G.E_MANAGER:add_event(Event({
      ...
      func = function()
        ...
        for i = 1, num_cards do
          ...
          -- The final 'true' here is the 'forced' flag to safely overwrite defaults.
...
SMODS.Consumable {
  ...
  abn_artist_credits = {
    artist = "Grass", -- Borrowing credit from eruption, change if needed
  },
}
    ...
    use = function(self, card, area, copier)
        ...
        if destroyed_count > 0 and #rarity_3_jokers > 0 then
            ...
            for _, j in ipairs(rarity_3_jokers) do
                ...
                --todo: figure out way to increase these jokers mult
    ...
    G.E_MANAGER:add_event(Event({
      ...
      func = function()
        ...
        for i = 1, num_cards do
          ...
          local chosen_front = #snow_fronts > 0 and pseudorandom_element(snow_fronts, pseudoseed('avalanche_front')) or
              pseudorandom_element(G.P_CARDS, pseudoseed('avalanche_fallback'))
