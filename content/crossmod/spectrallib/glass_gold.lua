  ...
  calculate = function(self, card, context)
    ...
    if context.before then
      ...
      for _, playing_card in ipairs(context.full_hand) do
        ...
        if has_gold and has_glass then break end
      end

      if has_gold and has_glass then
        for _, playing_card in ipairs(context.full_hand) do
          if SMODS.has_enhancement(playing_card, "m_glass") or SMODS.has_enhancement(playing_card, "m_gold") then
    ...
    if context.post_trigger and context.other_card then
      if context.other_card ~= card and context.other_card.ability then
        -- Glass Sticker
        if context.other_card.ability.abn_stk_glass or context.other_card.config.center == G.P_CENTERS.m_glass then
